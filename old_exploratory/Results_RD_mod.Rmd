...
library(gt)
# Get ANOVA table 
anova_table <- anova(model_F_R)

# Convert to tidy format
anova_tidy <- broom::tidy(anova_table)

# Display the tidy ANOVA table using gt with formatted p-values
anova_tidy %>%
  gt() %>%
...
```{r}
# model above by richness level and anova table
 main_r_dd %>%
  nest(data = -richness) %>%
  mutate(
    model = map(data, ~ lm(log10(1/CV) ~ log10(balance) * type, data = .x)),
    results = map(model, broom::tidy)
  ) %>%
  unnest(results) %>% dplyr::filter(term=="log10(balance):typerealised")
...
plot_int_stab<-ggplot(data=int_aggr%>%dplyr::filter(theta%in%c("none","var")),aes(x=mean_int_strength,y=-synchrony_Gross,color=nutrients))+
  geom_point()+
...
plot_pop_aggr_balance<-ggplot(data=pop_aggr, aes(y=-synchrony_Gross,x=log10(1/w_pop_CV)))+
  geom_point()+
