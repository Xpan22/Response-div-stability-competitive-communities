...
```{r R_squared, fig.align="center", fig.height=2, fig.width=8}
...
a <- ggplot(data = lm_divergence_richness_R, aes(x = richness, y = r.squared, color = "Divergence")) +
  geom_point( shape = 18, cex = 3 )  +
  geom_point(data = lm_balance_richness_R, aes(x = richness, y = r.squared, color = "Balance"), shape = 18, cex = 3 ) +
...
b <- ggplot(data = lm_divergence_richness_E, aes(x = richness, y = estimate, color = "Divergence") ) +
  geom_point(shape = 18, cex = 3) +
  geom_point(data = lm_balance_richness_E, aes(x = richness, y = abs(estimate), color = "Balance"), shape = 18, cex = 3 ) +
...
```{r model_check8, fig.cap='model check 1.', fig.align="center", fig.height=9, fig.width=12}
...
```{r async,  fig.align="center", fig.height=6, fig.width=8}
...
plot_int_stab<-ggplot(data=int_aggr%>%dplyr::filter(theta%in%c("none","var")),aes(x=mean,y=-synchrony_Gross,color=nutrients))+
  geom_point()+
  geom_smooth(method="lm",show.legend=F)+
  theme_classic() +
   theme(axis.title.x = element_text(size = 16),
...
pop_aggr<-complete_ts%>%group_by(sample_ID,predict_spec)%>%summarize(pop_CV=sd(biomass)/mean(biomass),mean_biom_contribution=mean(biom_contribution,na.rm=T))

# Lehman & Tilman (2000), THIBAUT & Connolli (2013), Gross et al. (2014)

pop_aggr<-pop_aggr%>%group_by(sample_ID)%>%summarize(w_pop_CV=sum(pop_CV*mean_biom_contribution),na.rm=T)
...
m1<-lm(data=pop_aggr,log10(stability)~log10(balance_f)+richness+log10(1/w_pop_CV)+temperature+nutrients)
check_model(m1)
summary(m1)
anova(m1)
...
model1C <- '
  ...
  log_pop_stab~log_balance_f+richness+nutrients+temperature
