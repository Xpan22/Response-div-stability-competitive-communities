...
```{r results='hide', echo=FALSE, warning=FALSE, message=FALSE}
interactions_df <- read_csv("Data/interactions_df_multi_thetas.csv")
...
```{r}
mixed_M_int<-lmer(data=int_aggr%>%dplyr::filter(theta%in%c("none",5)),log10(stability)~log10(imbalance_f)+nutrients+temperature+as.numeric(richness)+mean_interaction+(1|rep_var))
```
...
plot_int_stab<-ggplot(data=int_aggr%>%dplyr::filter(theta%in%c("none",5)),aes(x=mean_interaction,y=-synchrony_Gross,color=nutrients))+
  geom_point()+
  geom_smooth(method="lm",show.legend=F)+
  theme_classic() +
   theme(axis.title.x = element_text(size = 16),
