...
```{r}
lm_full<-lm(data=complete_aggr,log10(stability)~log10(imbalance_f)+richness+nutrients+temperature)
```
...
```{r model_check1, fig.cap='model check 1.', fig.align="center", fig.height=6, fig.width=10}
...
model_summary <- lm_full %>%
  ...
  dplyr::select(term, estimate, conf.low, conf.high, std.error, p.value) # Select relevant columns

# Create a publication-ready table with conditional bold formatting
table1 <- model_summary %>%
  kable("html", col.names = c("Term", "Estimate", "Lower 95% CI", "Upper 95% CI", "Std. Error", "p-value"),
        digits = 3, align = "c") %>%
...
```{r}
lm_full_w<-lm(data=complete_aggr,log10(stability)~log10(imbalance_r)+richness+nutrients+temperature)
...
```{r async_plots,results='hide', warning=FALSE, fig.cap='Relatioship temporal stability and asynchrony (Gross) divided by nutrient level.', fig.align="center", fig.height=6, fig.width=8}
...
```{r async, results='hide', warning=FALSE, fig.cap='Relatioship between asynchrony and fundamental imbalance divided by nutrient level.', fig.align="center", fig.height=6, fig.width=8}
