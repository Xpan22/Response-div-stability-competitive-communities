...
# Generate a tidy table with gt
tidy_results <- main_r_dd %>%
  nest(data = -richness) %>%
  mutate(
  ...
  unnest(results) %>%
  dplyr::filter(term == "log10(balance):typerealised") %>%  # Filter for interaction term
  dplyr::select(richness, term, estimate, std.error, statistic, p.value) %>%  # Select relevant columns
  dplyr::rename(
    Richness = richness,
    Term = term,
    Estimate = estimate,
  ...
  gt() %>%
  tab_header(title = "Regression Results for log10(balance):type (Realised)") %>%
  fmt_number(columns = vars(Estimate, Std_Error, T_Value, P_Value), decimals = 3) %>%
  tab_style(
    style = cell_text(weight = "bold"),
...
anova_table <- car::Anova(lm_full, type = "III")
...
anova_table <- car::Anova(lm_full_int, type = "III")
...
lm_full_int_2<-lm(data=complete_aggr_2,log10(stability)~scale(nutrients)*scale(temperature))

summary(lm_full_int_2)

lm_full_int_3<-lm(data=complete_aggr_2,log10(stability)~scale(nutrients)*scale(temperature)+log10(balance_f))

summary(lm_full_int_3)

car::Anova(lm_full_int_3,type="II")

group_env<-complete_aggr_2%>%nest(data=-treatment)%>%mutate(
    model = map(data, ~ lm(log10(1/CV) ~ log10(balance_r) , data = .x)),
    results = map(model, broom::glance)
  ) %>%
  unnest(results)
